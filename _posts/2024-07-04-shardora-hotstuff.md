...
由于对 `TimeoutMsg` 的广播，此视图切换方案需要 `O(n^2)` 的通讯复杂度，但提高了系统鲁棒性，是多数工程实践的选择，如 LibraBFT。比如如果不广播 `TimeoutMsg` 而直接发送给下一个 Leader，虽然可以降低通讯复杂度，但却造成了 censorship 隐患，如果下一个 Leader 生成 TC 后选择不广播，也不发起新的提案，那么整个系统就会丧失活性。因此更常见的方式是广播 `TimeoutMsg` 令所有节点本地生成 TC，触发视图切换，避免丧失活性的风险。
不过，我们仍然可以使用其他策略避免超时状态下 `O(n^2)` 的通讯复杂度，比如使用渐进式的 HighQC 同步策略，同时在 Leader 选择策略中增加超时信息，以保证活性，具体实现细节将在后面的「同步模块」中详细介绍。
